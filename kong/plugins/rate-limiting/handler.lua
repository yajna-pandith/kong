...
function RateLimitingHandler:access(conf)
  ...
  if usage then
    ...
    if not conf.hide_client_headers then
      ...
      local l
      for n=1, LIMITS_N do
        if limits[LIMITS[n]] then
          l = l or {}
          l[i+1] = ", "
          l[i+2] = limits[LIMITS[n]]
          l[i+3] = ";w="
          l[i+4] = EXPIRATIONS[LIMITS[n]]
          i=i+4
        end
      end

      -- enable rate-limiting windows information only
      -- when there is more than one window enabled
      if i > 5 then
        l[1] = limit
        limit = concat(l)
      end
